...
**Architecture:** Two pieces. (A) The **engine** — modified `AutoRaise.mm` — adds a pure display-gating function and hooks it into `onTick()` so the raise logic only runs while "armed" by a display crossing. (B) The **launcher** — a Swift menu-bar app built with SwiftPM and assembled into `AutoRaise.app` by a shell script — bundles the engine binary, writes the delay into the engine's config file, and runs the engine argless as a subprocess.

**Tech Stack:** Objective-C++ (engine, built with `make`), Swift 6 + AppKit + ServiceManagement (launcher, built with SwiftPM — works under Command Line Tools, no full Xcode required), a `make-app.sh` bundle-assembly script, and `codesign` with a self-signed identity (ad-hoc fallback).
...
## Task 3: Launcher scaffold (SwiftPM + bundle script)

Stands up the buildable app shell with SwiftPM (no full Xcode needed): a `Package.swift` executable target for the launcher, a minimal menu-bar `AppDelegate`, a static `Info.plist`, and a `make-app.sh` that builds the engine (`make`) + launcher (`swift build`) and assembles + signs `AutoRaise.app`.

**Files:**
- Create: `Package.swift`
- Create: `Launcher/main.swift` (explicit entry point)
- Create: `Launcher/AppDelegate.swift` (minimal)
- Create: `Launcher/Info.plist`
- Create: `make-app.sh`
- Modify: `.gitignore`

**Interfaces:**
- Produces: an `AutoRaise.app` bundle (in repo root) with `Contents/MacOS/AutoRaise` (launcher) and `Contents/MacOS/AutoRaiseEngine` (engine). SwiftPM builds the launcher executable as `AutoRaiseLauncher` under `.build/release/`.

- [ ] **Step 1 (optional, one-time): create a stable self-signed code-signing certificate**

Recommended so the Accessibility grant persists across rebuilds. In **Keychain Access ▸ Certificate Assistant ▸ Create a Certificate…**: Name `AutoRaise Self-Signed`, Identity Type **Self-Signed Root**, Certificate Type **Code Signing**. If skipped, `make-app.sh` falls back to ad-hoc signing (grant resets each rebuild). Verify: `security find-identity -v -p codesigning | grep "AutoRaise Self-Signed"`.
...
- [ ] **Step 3: Write the entry point and the minimal `AppDelegate.swift`**

Create `Launcher/main.swift`:
...
- [ ] **Step 4: Write `Launcher/Info.plist`**
...
<plist version="1.0">
<dict>
    <key>CFBundleExecutable</key>       <string>AutoRaise</string>
    <key>CFBundleIdentifier</key>       <string>local.autoraise.displayfocus</string>
    <key>CFBundleName</key>             <string>AutoRaise</string>
    <key>CFBundleDisplayName</key>      <string>AutoRaise</string>
    <key>CFBundlePackageType</key>      <string>APPL</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundleVersion</key>          <string>1</string>
    <key>CFBundleIconFile</key>         <string>AutoRaise</string>
...
Create `make-app.sh` (make it executable in Step 6):
...
- [ ] **Step 6: gitignore build output and make the script executable**
...
- [ ] **Step 7: Build the launcher executable**

Run: `swift build -c release`
Expected: `Build complete!` (compiles the `AutoRaiseLauncher` target; `Launcher/Tests` is excluded).

- [ ] **Step 8: Assemble the app bundle**

Run: `./make-app.sh`
Expected: ends with `Built AutoRaise.app (signed with: …)`.

- [ ] **Step 9: Verify the bundle contains both binaries**

Run: `ls AutoRaise.app/Contents/MacOS`
Expected: two files — `AutoRaise` and `AutoRaiseEngine`.

- [ ] **Step 10: Verify it launches as a menu-bar app**

Run: `open AutoRaise.app`
Expected: a menu-bar icon appears (no Dock icon), and its menu shows **Quit AutoRaise**. Use Quit to exit.

- [ ] **Step 11: Commit**

```bash
git add Package.swift Launcher/main.swift Launcher/AppDelegate.swift Launcher/Info.plist make-app.sh .gitignore
git commit -m "feat(launcher): SwiftPM scaffold + bundle-assembly script"
```
...
Run: `swift build -c release`
Expected: `Build complete!` (Tests/ is excluded from the app target, so no top-level-code conflict).
...
Run: `./make-app.sh && open AutoRaise.app`
...
Run: `./make-app.sh && rm -rf /Applications/AutoRaise.app && cp -R AutoRaise.app /Applications/ && open /Applications/AutoRaise.app`
...
- The engine and the launcher are signed with the **same** identity by `make-app.sh` (prefers `AutoRaise Self-Signed`, falls back to ad-hoc `-`). If you rename the certificate, update the `IDENTITY` variable in `make-app.sh`.
- `AutoRaise.icns` exists in the repo; `make-app.sh` copies it into `Contents/Resources` and `Info.plist` references it via `CFBundleIconFile` — not required for functionality.
- SwiftPM builds under Command Line Tools (no full Xcode). If `swift build` ever reports a missing SDK, run `xcode-select -p` to confirm a toolchain is active.
- Ad-hoc signing works for local use, but the Accessibility grant resets on each rebuild because the code hash changes. Create the `AutoRaise Self-Signed` cert (Task 3 Step 1) to keep the grant stable across rebuilds.
